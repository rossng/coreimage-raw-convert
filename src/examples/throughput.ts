  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '-h':
        ...
        console.log('Options:');
        console.log('  --conversions, -c <num>  Number of conversions to perform (default: 100)');
        console.log('  --concurrency, -j <num>  Concurrency level for async test (default: 4)');
...
async function runConcurrentTest(
  ...
  console.log(`  Running ${conversions} concurrent conversions (concurrency: ${concurrency})...`);
...
async function runThroughputBenchmark(): Promise<void> {
  ...
  console.log(`RAW image size: ${(rawBuffer.length / 1024 / 1024).toFixed(2)} MB\n`);
  ...
  const sequentialResult = await runSequentialTest(rawBuffer, options.conversions);
  console.log(`  Total: ${sequentialResult.totalTime.toFixed(0)}ms, Avg: ${sequentialResult.avgTime.toFixed(2)}ms, Throughput: ${sequentialResult.throughput.toFixed(2)} conv/s\n`);
  
  // Concurrent test
  console.log('📊 Concurrent Processing');
  const concurrentResult = await runConcurrentTest(rawBuffer, options.conversions, options.concurrency);
  console.log(`  Total: ${concurrentResult.totalTime.toFixed(0)}ms, Avg: ${concurrentResult.avgTime.toFixed(2)}ms, Throughput: ${concurrentResult.throughput.toFixed(2)} conv/s\n`);
  ...
  console.log('-'.repeat(80));
  console.log(`Sequential    ${sequentialResult.totalTime.toFixed(0).padStart(8)}ms    ${sequentialResult.avgTime.toFixed(2).padStart(7)}ms    ${sequentialResult.throughput.toFixed(2).padStart(7)} c/s`);
  console.log(`Concurrent    ${concurrentResult.totalTime.toFixed(0).padStart(8)}ms    ${concurrentResult.avgTime.toFixed(2).padStart(7)}ms    ${concurrentResult.throughput.toFixed(2).padStart(7)} c/s`);
  console.log('='.repeat(80));
  console.log(`Speedup: ${speedup.toFixed(2)}x faster with concurrency ${options.concurrency}`);
  console.log(`Total conversions: ${options.conversions * 2} (${options.conversions} sequential + ${options.conversions} concurrent)`);
  console.log(`Best throughput: ${Math.max(sequentialResult.throughput, concurrentResult.throughput).toFixed(2)} conversions/second`);
  console.log('');
}
